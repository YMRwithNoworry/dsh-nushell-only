...
import { isQuotedHead, rawStringEnd, splitStatements, startsComment, tokenize } from './guard.js'
...
const RULES = [
  ...
  },
  {
    ...
    find: (view, command) => matchFragment(view, /(?:^|[\s(])-not\b/i, command),
  },
  {
    id: 'deprecated-flags',
  ...
  },
  {
    ...
    find: (view, command) => matchFragment(view, /(?:^|[\s|(])-(?:ErrorAction|ErrorVariable|WarningAction|Recurse|LiteralPath|Filter|Confirm|Force)\b/i, command),
  },
  {
    id: 'negative-count',
  ...
  },
  {
    ...
    find: (view) => matchFragment(view, /\|\s*(?:o\+e>|out\+err>|o>>|out>>|o>|out>|e>>|err>>|e>|err>)/),
  },
  {
    id: 'stdout-redirect',
    detail: 'In Nushell `>` is *comparison*, not redirection, so this is not a redirect at all: Nushell reads the operator and the filename as two more arguments, exits 0, and writes no file (silent misbehaviour, like `2>$null`). Redirect stdout with `cmd o> out.txt`, append with `cmd o>> out.txt`, both streams with `cmd o+e> out.txt`, or `cmd | save -f out.txt`.',
    find: (view) => matchFragment(view, /(?:^|[\s;|&(])>>?\s+[^\s;&|(){}]+(?:\.[A-Za-z0-9]{1,8}\b|\/|\\|:|^nul$)/),
  },
]
...
function findNestedForeignProgram(command) {
  ...
  const defined = definedCommands(view)
  for (const match of view.matchAll(/(?:^|[\n;|&({])\s*([A-Za-z_][A-Za-z0-9_.\\/-]*)/g)) {
    const token = match[1]
    const offset = match.index + match[0].length - token.length
    const next = view[offset + token.length]
    // `{ get-content: 1 }` is a record whose key happens to be a cmdlet name.
    if (next === ':') continue
    const base = token.split(/[\\/]/).pop() ?? token
    const name = base.toLowerCase().replace(/\.(exe|cmd|bat|com)$/, '')
    const cmdlet = POWERSHELL_CMDLETS.get(name)
    if (cmdlet === undefined || defined.has(name)) continue
    return {
...
/**
 * Flags of `glob` that take an operand, so the operand is not a second pattern.
 * Bounding a walk with a depth is correct Nushell — and the refusal for an extra
 * positional used to recommend exactly that spelling while refusing it.
 */
const GLOB_OPERAND_FLAGS = new Set(['-d', '--depth', '-e', '--exclude'])

/**
 * The positional (non-flag) tokens of a `glob` call, skipping each flag's
 * operand — including an operand written as a bracketed or parenthesised
 * expression, which `tokenize` splits into several tokens.
 * @param tokens - the statement's tokens (the first is `glob`).
 * @returns the positional tokens.
 */
function globPositionals(tokens) {
  const positionals = []
  for (let index = 1; index < tokens.length; index++) {
    const token = tokens[index]
    if (token.startsWith('-')) {
      const flag = token.split('=')[0].toLowerCase()
      if (GLOB_OPERAND_FLAGS.has(flag) && !token.includes('=')) {
        const operand = tokens[index + 1]
        index++
        if (operand !== undefined && /^[([{]/.test(operand) && !/[)\]}]$/.test(operand)) {
          while (index + 1 < tokens.length && !/[)\]}]$/.test(tokens[index])) index++
        }
      }
      continue
    }
...
/**
 * Command names this command defines itself (`def get-content [] { … }`,
 * `alias ls = eza`). A user-defined name shadows a PowerShell cmdlet-name
 * lookup, and refusing a call to the model's own command is a false positive.
 * @param view - the code view of the command.
 * @returns the lowercased defined names.
 */
function definedCommands(view) {
  const names = new Set()
  for (const match of view.matchAll(/\b(?:export\s+)?(?:def|alias)\s+([A-Za-z_][A-Za-z0-9_-]*)/g)) {
 ...
 * real program is ever named `Get-Content` unless the command defines one) and
 * `glob` argument misuse. Unix tools are deliberately absent — they are
 * hint-only, because on POSIX they are legitimate external programs.
 * @param command - the Nushell source.
 * @param defined - names the command defines itself.
 * @returns the first finding, or undefined.
 */
function findForeignProgram(command, defined) {
  let cursor = 0
  for (const statement of splitStatements(command)) {
    ...
    const name = base.toLowerCase().replace(/\.(exe|cmd|bat|com)$/, '')
    const cmdlet = POWERSHELL_CMDLETS.get(name)
    if (cmdlet !== undefined && !defined.has(name)) {
      return {
        id: 'powershell-cmdlet',
    ...
    if (name === 'glob') {
      const positional = globPositionals(tokens)
...
function findBareAssignment(view, command) {
  ...
  // `mut x: int = 0` declares `x` just as `let x = 0` does; the type annotation
  // group stops at a delimiter so `def f [mut x: int] { … }` (where `mut` does
  // not make the local mutable — Nushell still errors) is not read as one.
...
/**
 * Whether this offset sits inside a statement whose program is an explicit
 * external (`^node …`, `&node …`). The flags after `^prog` belong to that
 * program, not to Nushell: `^node -e '…' -- -Force` hands `-Force` to node, so a
 * rule that matches PowerShell flag *names* must not fire there.
 * @param command - the Nushell source.
 * @param offset - the offset to test.
 * @returns true when the offset is inside an explicit-external statement.
 */
function insideExplicitExternal(command, offset) {
  let cursor = 0
  for (const statement of splitStatements(command)) {
    const start = command.indexOf(statement, cursor)
    if (start === -1) continue
    cursor = start + statement.length
    if (offset < start || offset >= cursor) continue
    const first = tokenize(statement)[0] ?? ''
    return first.startsWith('^') || (first.startsWith('&') && first.length > 1)
  }
  return false
}

/** Locate a regex match and return it as a rule finding. */
function matchFragment(view, pattern, command) {
  const flags = pattern.flags.includes('g') ? pattern.flags : `${pattern.flags}g`
  const global = new RegExp(pattern.source, flags)
  for (const match of view.matchAll(global)) {
    if (command !== undefined && insideExplicitExternal(command, match.index)) continue
...
export function findDialectIssue(command) {
  ...
  const foreign = findForeignProgram(command, definedCommands(view))
...
const HINTS = [
  ...
  },
  {
    ...
    text: 'Assignments need `let x = …` / `mut x = …` (a bare `$x = …` is invalid), `$(…)` is not a subexpression — use `(…)`, and loops are `for x in [...] { … }`, not `foreach`. A shell variable is not a Nushell variable either: `echo $HOME` wants `print $env.HOME`.',
  },
  {
    id: 'env-var-not-var',
    match: /nu::parser::env_var_not_var/,
    text: '`$PATH`, `$HOME`, `$USERPROFILE` are shell variables, not Nushell ones. Nushell reads the environment as `$env.PATH`, `$env.HOME` (optional read: `$env.NAME?`; write: `$env.NAME = "value"`), and lists the real names with `$env | columns`.',
  },
  {
  ...
  {
    id: 'condition-not-boolean',
    match: /can't convert[^\n]*to (?:a )?bool|Cannot convert[^\n]*to (?:a )?bool/i,
    text: 'A condition must be a boolean. `[ -f x ]` is a *list* in Nushell, not a test: write `if (\'x\' | path exists) { … }`, `if ($x | is-empty) { … }`, or compare directly (`if $n > 3 { … }`).',
  },
  {
    id: 'input-type',
  ...
  },
  {
    ...
    text: 'That pattern matched nothing (and `ls <glob>` treats that as an error). Use `glob <pattern>` — it returns an empty list — or widen the pattern (`glob "D:/dir/**/*.rs"`, `--depth N`). `?` in a path is a one-character glob wildcard in Nushell (not bash\'s "optional"), so a bare `ls dir/file.tx?` also expands and then fails when nothing matches.',
  },
  {
...
export const DIALECT_RULE_IDS = [...RULES.map((rule) => rule.id), 'bare-assignment', 'prefix-assignment', 'powershell-cmdlet', 'glob-extra-positional', 'glob-named-arguments']
