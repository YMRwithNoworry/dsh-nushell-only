...
function errorSection() {
  return [
    ...
    '',
    'Nushell parses a whole command before running it, so a single dialect slip fails the entire call and `try` cannot catch it. The rows below are ordered by how often they were observed in a real Nu-only dsh deployment; the preflight check refuses most of the left column before it ever spawns, and everything else comes back with a `Nushell hint (…)` line.',
    '',
    '| you wrote | Nushell reported | write this instead |',
    '|---|---|---|',
    '| `cmd 2>&1`, `cmd 2>$null`, `cmd 2>/dev/null` | `nu::parser::shell_outerr` | `cmd o+e> out.txt` (both), `cmd e> err.txt`, `cmd o> out.txt`; to capture: `do { cmd } \\| complete` |',
    ...
    '| `select -First 20`, `get -i col`, `str downcase` | `nu::parser::unknown_flag`, `deprecated` | `first 20`, `get -o col`, `str lowercase` |',
    ...
    '```',
    '',
    'Habits worth naming: `2>&1` is `o+e>`, `$x = 1` needs `let`, `$env:NAME` is `$env.NAME`, `@(…)` is `[…]`, `foreach` is `for … in`, PowerShell cmdlets have one-word Nu equivalents, and `ls` / `open` fail loudly on a missing path instead of printing a shell error.',
  ].join('\n')
}
