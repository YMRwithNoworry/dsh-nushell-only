...
function errorSection() {
  return [
    ...
    '| `ls \'~/.dsh/profiles?\'` | `No matches found for Expand(…)` | `?` in a path *is* a one-character glob wildcard (bash reads it as "optional"), and `ls <glob>` errors when nothing matches: use `glob <pattern>` (empty list) or `<p> \\| path exists` |',
    '| `cmd > out.txt`, `cmd >> log.txt` | *no error* — `>` is **comparison** in Nushell, so the operator and the filename become two more arguments, no file is written and the call exits 0 | `cmd o> out.txt`, append with `cmd o>> log.txt`, both streams with `cmd o+e> out.txt`, or `cmd \\| save -f out.txt` |',
    '| `if [ -f x ] { … }`, `if [ -z "$x" ]` | `nu::shell::cant_convert` ("can\'t convert list<string> to boolean") | `[ -f x ]` is a *list*: write `if (\'x\' \\| path exists) { … }`, `if ($x \\| is-empty) { … }` |',
    '| `echo $HOME`, `print $PATH` | `nu::parser::variable_not_found`, `env_var_not_var` | shell variables do not exist: `$env.HOME`, `$env.PATH` (optional `$env.NAME?`) |',
...
function gotchaSection() {
  return [
    ...
    '- A bare `>` is comparison, never redirection: `cmd > out.txt` writes nothing and exits 0. Redirect with `o>` (append `o>>`), `e>`, `o+e>`, or `| save -f`.',
    '- A closure parameter may be named anything — `each { |sh| … }` is fine (only a *pipeline* position runs a program), and `glob --depth 2 **/*.txt` is one pattern plus a flag operand.',
